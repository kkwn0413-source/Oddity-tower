...
- `/login`: 매직링크. 가입은 director 초대(profiles 사전 생성).
...
2. Supabase 마이그레이션(스키마 전체) + RLS + 시드
3. Auth(매직링크) + profiles + director/freelancer 가드
4. 타임라인 읽기전용 렌더(프로토타입 이식) + 프로젝트별/인원별 축 전환
5. 태스크 상세 패널 — 상태변경·코멘트·director 생성/수정
6. 파일 업로드(자동네이밍·버전) + Drive/Figma 링크 + signed URL
7. 레퍼런스 보드 (조회→업로드→큐레이션→판정 RPC/모아보기→doc_group→방향로그→Realtime)
