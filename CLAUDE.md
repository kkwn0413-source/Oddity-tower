...
- `/files`: 보고 파일 (내부용, 사용자 확장 2026-07-08) — approved=true 파일만 업체→프로젝트 그룹으로 모아보기. 이미지는 signed URL 썸네일+라이트박스, 기타는 새 탭. 업체 필터 칩, director는 "보고 제외"(approved 해제). RLS 범위 그대로(프리랜서는 자기 범위만).
- `/admin/team`: 팀 관리 (director, 사용자 확장 2026-07-08) — 계정 발급(초기 비밀번호 1회 표시)·이름/색상 수정·비밀번호 재발급·비활성화(ban). service role API `/api/admin/team`. 본인 비밀번호 변경은 `/account`(헤더 이름 클릭).
...
6. ✅ 파일 업로드(자동네이밍·버전) + Drive/Figma 링크 + signed URL — storage RLS는 0009, 표시 이름은 task_files.name(경로는 {task_id}/{uuid}). 팀 이름 표시는 team_directory() RPC 사용(직접 SELECT는 freelancer에게 본인 row만 내려옴)
